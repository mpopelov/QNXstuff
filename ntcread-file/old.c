...
int main()
{
	...
	if( !(ntc_fd = open("session.log", O_RDONLY)) ){
		printf("Can't open /dev/ntc for logging.\n");
		return (-1);
	}
	...
	/* get info on interfaces */
	{
		...
		int result = devctl(fd, NTC_IFPRESENT, ifdesc, 5*sizeof(if_info_t), &num_ifs );
		if( EOK != result ){
		...
		/* get info on interfaces */
	{
		...
		result = devctl(fd, NTC_ADDIF, ifname, IFNAMSIZ, &rslt );
		if( EOK != result ){
	...
	while(1){
		...
		size_read = read(fd, packet, MAX_PKT_READ*sizeof(pkt_info_t));
		if(size_read == -1){
			printf("device lost\n");
			close(fd);
			return 0;
		...
		if(size_read == 0){
			 ...
			 * for some time
			 */
			result = devctl( fd, NTC_NUMMISSED, &somenumber, sizeof(int), NULL );
			if(EOK != result){
			...
			result = devctl( fd, NTC_ADDBLOCK, &mac, sizeof(mac_addr_t), NULL );
			if(EOK != result){
				printf("We've lost device!\n");
				close(fd);
				return 0;
			...
			result = devctl( fd, NTC_DELBLOCK, &mac, sizeof(mac_addr_t), NULL );
			if(EOK != result){
				printf("We've lost device!\n");
				close(fd);
				return 0;
			...
			result = devctl( fd, NTC_NUMBLOCKED, &blocked, sizeof(blkinfo_t), NULL );
			if(EOK != result){
				printf("We've lost device!\n");
				close(fd);
				return 0;
